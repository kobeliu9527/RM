...
namespace Shared.Extensions
{
    ...
    public static class ContainerDtoExtensions
    {
            ...
            foreach (var row in container.Rows)
            {
                foreach (var com in row.ComponentList)
                {
                    ...
                    if (com.ChildContainers?.Count > 0)
                    {
                        ...
                        {
                            c.FindComponent(match, ref component);
                        }
                    }
                }
            }
        ...
        {
            var root = new TreeViewItem<TreeComponentData>(new TreeComponentData() { Key = container.Id, Title = container.Name, Type = ControlType.Comtainer, Children = new List<TreeComponentData>() })
            {
                Text = container.Name
            };
